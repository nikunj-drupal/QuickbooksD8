...
class QuickBooksSupport {
   ...
   * @var string
   */
  public static $oAuthUrl = 'https://ji-quickbooks.joshideas.com/ji_oauth_authorize';
    ...
    if (!isset($option_agencies)) {
      ...
      if ($cache) {
        ...
        try {
          ...
          \Drupal::logger('ji_quickbooks')->critical(t('Attempting to cache tax agencies failed, exception thrown: @e', ['@e' => $e->getMessage()]));
          return TRUE;
        }
  ...
  public static function checkIfSettingsApplied($show_message = FALSE) {
    $settings_count = 0;
    \Drupal::state()->get('ji_quickbooks_payment_method') ? $settings_count++ : NULL;
    \Drupal::state()->get('ji_quickbooks_payment_account') ? $settings_count++ : NULL;
    \Drupal::state()->get('ji_quickbooks_term') ? $settings_count++ : NULL;
    \Drupal::state()->get('ji_quickbooks_default_product') ? $settings_count++ : NULL;
    if (empty($settings_count) || $settings_count < 4) {
      if (\Drupal::currentUser()->hasPermission('access quickbooks')) {
        if ($show_message) {
          drupal_set_message(t('JI QuickBooks is missing the one or more settings. Please make adjustments or save before you use it. Visit the <a href="@url">settings page</a>.', ['@url' => url('admin/config/services/ji_quickbooks')]), 'error', FALSE);
        }
      }

      $url = Url::fromRoute('ji_quickbooks.form')->toString();
      \Drupal::logger('JI QuickBooks')->error("JI QuickBooks is missing the one or more settings. Please make adjustments or save before you use it. Visit the <a href='$url'>settings page</a>.");
   ...
   */
  public static function getLibraryPath($name, $base_path = FALSE) {
